...
```powershell
powershell -File "$env:USERPROFILE\.agents/skills/acad-recreation\tools\install.ps1"
```
...
явный; + бэкап), печатает строку автозагрузки для `acad.lsp`. **После патча — перезапустить OpenCode.** Автозагрузку toolkit
...
- **Безопасный блок:** `(ORG:safe-run '(...))` — оборачивает геометрию в `UNDO _GROUP … _END` +
  финальный `ZOOM _E`, чтобы один откат снимал весь шаг (канонический скелет из OpenCodeCAD-методологии).
