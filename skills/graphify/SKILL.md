...
У graphify 40+ команд/флагов, и три из них легко жгут токены или ломают OpenCode. Перед запуском:
...
3. **Не ставь ломающий хук вслепую.** `graphify OpenCode install` вшивает PreToolUse-хук, который на
   части сборок OpenCode слетает и **блокирует чтение файлов**. НЕ ставить по своей инициативе;
   для авто-обновления рекомендуй безопасный **post-commit** (`graphify hook install`). Откат, если
   хук уже сломал чтение: удалить блок `hooks` с `graphify` из `settings.json` → перезапустить OpenCode.
...
> **Even on the fast path, do NOT skip vocab-expansion.** `## For /graphify query` begins with a REQUIRED Step 0 (constrained vocab-expansion, see `references/query.md`). graphify's matcher has no stemming/synonyms/cross-language — a Russian question against English or normalized labels (our OpenCode-base case) returns **0 hits** without expansion. Expand first, then query. "Run immediately" means "don't rebuild", not "skip expansion".
...
> (gitignore-синтаксис) в корне сканирования — наряду с `.gitignore`. Для графа OpenCode-base такой файл
> уже лежит в `~/.config/opencode/base/.graphifyignore` и исключает session-reports/harvested/evals/служебные каталоги:
> без него граф засорялся на ~55% (мусорные узлы топят ядро, god-nodes садятся на отчёты вместо
> agents/skills/AGENTS.md). Строя граф ЛЮБОГО проекта с «версионируемым, но не-структурным» контентом
...
chunks = sorted(glob.core.search.files('graphify-out/.graphify_chunk_*.json'))
...
> **Windows hub / OpenCode-base:** on a Windows host with a Cyrillic install path, do NOT
...
[[wikilinks]], name mentions, frontmatter, AGENTS.md sections. Rebuilt automatically by the
...
enrichment) on a Windows hub with a Cyrillic install path (OpenCode-base / CYRILLIC-HOST). The LLM extraction (Step 3B
