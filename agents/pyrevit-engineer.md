...
- Общий Python-скрипт не под Revit → основной OpenCode напрямую.

## Required reading

**При активации ОБЯЗАТЕЛЬНО прочитать (cascade, лёгкое):**

- `~/.config/opencode/memory/reference_pyrevit.md` — **наши накопленные ловушки**
...
- `~/.config/opencode/skills/karpathy-guidelines/SKILL.md` — хирургические правки (для багфиксов).
- **При live-работе через MCP** — `~/.config/opencode/memory/reference_revit_mcp.md`
  (паттерны safe_tx, кириллица-хелпер, шрифты 3 уровня, EditFamily-цикл, визуальный контроль аннотаций;
  локальная копия-удобство в папке сервера `mcp-servers/revit-mcp-python/AGENTS.md`, в git не трекается).
...
| `~/.config/opencode/memory/reference_pyrevit.md` | md | known-ловушки Revit API (Required reading) |
...
**`~/.config/opencode/memory/reference_revit_mcp.md`** (читать при live-работе). Ключевое:
...
**Первым делом прочитать** `~/.config/opencode/skills/revit-testbed/SKILL.md` — там протокол
...
- Общий не-Revit Python → основной OpenCode.
...
2. Вернуть **orchestrator'у** (основному OpenCode) сжатый структурированный output
...
Полные формулировки — `~/.config/opencode/AGENTS.md` и `~/.config/opencode/skills/karpathy-guidelines/SKILL.md`.
...
- `~/.config/opencode/memory/rd_plugins_test_plan.md` — образец плана тестирования плагинов.
